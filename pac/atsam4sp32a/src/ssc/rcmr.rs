...
pub enum CKG_A {
    #[doc = "0: None"]
    CONTINUOUS = 0,
    #[doc = "1: Receive Clock enabled only if RF Pin is Low"]
    EN_RF_LOW = 1,
    #[doc = "2: Receive Clock enabled only if RF Pin is High"]
    EN_RF_HIGH = 2,
}
...
impl<'a> CKG_W<'a> {
    ...
    #[doc = "Receive Clock enabled only if RF Pin is Low"]
    #[inline(always)]
    pub fn en_rf_low(self) -> &'a mut W {
        self.variant(CKG_A::EN_RF_LOW)
    }
    #[doc = "Receive Clock enabled only if RF Pin is High"]
    #[inline(always)]
